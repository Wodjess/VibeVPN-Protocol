...
import * as VPN from '../utils/vpn';
import { loadServers, saveServers } from '../utils/storage';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const unsub = VPN.onStatusChange(({ status: s }) => {
      setStatus(s);
    });
    // Initial status
    VPN.getStatus().then(s => {
      setStatus(s.status);
  ...
  const handleAddServer = useCallback(async (server) => {
  ...
  const handleRemoveServer = useCallback(async (id) => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {status === 'connected' && peers.length > 0 && (
        <View style={styles.peersSection}>
          ...
          {peers.map(p => (
            <View key={p.ip} style={styles.peerItem}>
